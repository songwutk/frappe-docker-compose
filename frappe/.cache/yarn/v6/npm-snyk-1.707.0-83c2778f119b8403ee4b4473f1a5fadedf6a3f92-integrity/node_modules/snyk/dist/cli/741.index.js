...
function setSnykFixAnalytics(fixSummary, meta, snykTestResponses, resultsByPlugin, vulnerableResults) {
    ...
    for (const plugin of Object.keys(resultsByPlugin)) {
        ...
        if (failedToFix.length > 0) {
            errors.push(...failedToFix.map((f) => { var _a; return (_a = f.error) === null || _a === void 0 ? void 0 : _a.message; }));
        }
...
const total_issues_count_1 = __webpack_require__(41160);
const format_successful_item_1 = __webpack_require__(53689);
...
function throwPipenvError(stderr, command) {
    ...
    for (const error of errorsToBubbleUp) {
        if (errorStr.includes(error)) {
            throw new command_failed_to_run_error_1.CommandFailedError(stderr, command);
        }
    }
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.generateUpgrades = exports.updateDependencies = void 0;
const pathLib = __webpack_require__(85622);
...
const validate_required_data_1 = __webpack_require__(57894);
const standardize_package_name_1 = __webpack_require__(78078);
const attempted_changes_summary_1 = __webpack_require__(3616);
const debug = debugLib('snyk-fix:python:Poetry');
async function updateDependencies(entity, options) {
    const handlerResult = await fixAll(entity, options);
    return handlerResult;
...
async function fixAll(entity, options) {
    ...
    try {
        ...
        if (devUpgrades.length) {
            ...
            changes.push(...(await poetryAdd(entity, options, devUpgrades, installDev)));
        }
        if (!changes.length || !changes.some((c) => attempted_changes_summary_1.isSuccessfulChange(c))) {
